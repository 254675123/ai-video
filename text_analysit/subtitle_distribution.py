...
class Text2KnowledgeDistribution:
    ...
    def readJsonFileContent(self, filepath):
        f = open(filepath, 'r', encoding='utf-8')
        content = f.read()
        # 内容是json格式，需要对其转换成json对象
        flag = self.json_parser.parseJson(content)
        return flag

    def compute_similarity(self, course_base_code, video_file_name):
        ...
        print('开始预测')
        # 测试数据
        text_filepath = self.curPath+'/../data/course_json/{}.json'.format(video_file_name)
        parse_flag = self.readJsonFileContent(text_filepath)
        if parse_flag == False:
            print('解析json数据失败')
            return
        for line in self.json_parser.jsondata:
            item_time = line['start']
            item_content = line['content']
            content_words = self.splitor.split1list(item_content)
            sims = self.docvector.test_doc2vec(content_words)

            print('{}的对应的知识点如下：'.format(item_content))
            sim_kwg_list = []
            for sim in sims:
            ...
            self.time_kwg_dict[item_time] = sim_kwg_list

    def statistics(self):
        img_index_kwg_list = []
        for key, value in self.time_kwg_dict.items():
    ...
    def run(self, need_processed_video):
        ...
        self.compute_similarity(course_base_code, video_file)
...
if __name__ == '__main__':
    ...
    need_processed_video = ('open.bc.69','kuaiji-09.mp4')
    t2kd.run(need_processed_video)
    pass